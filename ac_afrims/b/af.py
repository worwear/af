...
class fte_tracking(osv.osv):
    ...
    def search(self, cr, uid, args, offset=0, limit=None, order=None, context=None, count=False):
        ...
        if period_start != '' and period_end != '':
            cr.execute('''select id from fte_tracking where (period_start >= '%s' and period_start <= '%s') or (period_end >= '%s' and period_end <= '%s') '''%(period_start,period_end,period_start,period_end) )
            fte_id = map(lambda x:x[0] , cr.fetchall())
            args = []
            args = [('id','in',fte_id)]
        res = super(fte_tracking, self).search(cr, uid, args, offset, limit, order, context, count)
        return res
    ...
    def send_requests(self2,self,cr,uid,ids):
        ...
        for obj in self.browse(cr,uid,ids):
            for dec in self2.browse(cr,uid,dec_ids):
                ...
                #print "conditions of decision '%s' satisfied"%dec.name
                for decider in dec.deciders:
                    print "  0.decider is",decider.name
                    if not decider.send_email:
                        print "   1. don't send email"
                        continue
                    if decider.condition and not eval(decider.condition,{"object":obj}):
                        print "    2.condition not pass"
                        continue
